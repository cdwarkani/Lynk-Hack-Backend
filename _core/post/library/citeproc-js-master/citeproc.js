...
CSL.getBibliographyEntries = function (bibsection) {
    ...
    for (var i = 0, ilen = input.length; i < ilen; i += 1) {
        ...
        entry_item_ids = [];
        if (this && this.registry && this.registry.registry && item && item.id && this.registry.registry[item.id] && this.registry.registry[item.id].master
        ...
        } else if (item && item.id && this.registry.registry[item.id] && !this.registry.registry[item.id].siblings) {
            this.parallel.StartCitation(sortedItems);
            this.tmp.term_predecessor = false;
            entry_item_ids.push("" + CSL.getCite.call(this, item));
            if (bibsection && bibsection.page_start && bibsection.page_length) {
...
CSL.NameOutput.prototype._isRomanesque = function (name) {
    var ret = 2;
    if (name && name.family && !name.family.replace(/\"/g, '').match(CSL.ROMANESQUE_REGEXP)) {
        ret = 0;
    }
    if (!ret && name && name.given && name.given.match(CSL.STARTSWITH_ROMANESQUE_REGEXP)) {
        ret = 1;
    }
    var top_locale;
    if (ret == 2) {
...
CSL.NameOutput.prototype._renderOnePersonalName = function (value, pos, i, j) {
    ...
    if (romanesque === 0) {
        ...
        if (second && second.strings && second.strings.prefix) {
                ...
                if (this.variables_real[0] === "citation-number") {
                    ...
                    func = function (state, Item, item) {
                        ...
                        if (!state.tmp.just_looking) {
                            ...
                            if (state.registry.registry && state.registry.registry[id] && state.registry.registry[id].seq)
...
CSL.Transform = function (state) {
    ...
    function quashCheck(value) {
        var m = undefined;
        if ((typeof value) == "string") { m = value.match(/^!([-,_a-z]+)>>>/); }
        if (m) {
